...
class GithubRepository:
    ...
    def get_stats(data: Union[List[Issue], List[PullRequest]]) -> Dict:
        ...
        if data:
            ...
            stats["open"] = len([x for x in data if x.state == 'open'])
            stats["old"] = len([x for x in data if x.is_old])
    ...
    def get_report(self, start_date: date, end_date: date) -> str:
        ...
        if contributors:
            report += "\n\u2B24 Таблица самых активных участников:\n"

            column_width = max([len(x.login) for x in contributors])

            report += f"+{'--position--'}+{'login'.center(column_width + 10, '-')}+{'contributions'.center(17, '-')}+\n"
            for i, contributor in enumerate(contributors):
                report += f"|{str(i+1).center(12, ' ')}"\
                    f"|{contributor.login.center(column_width + 10, ' ')}"\
                    f"|{str(contributor.contributions).center(17, ' ')}|\n"
            report += f"+{'-' * column_width + '-' * 41}+\n"

        report += f"\n\u2B24 Статистика по PullRequests:\n"
        report += "Всего - {total}\nОткрытых - {open}\nСтарых - {old}\nЗакрытых - {closed}\n".format(
            **self.get_stats(self.pull_requests)
        )

        report += f"\n\u2B24 Статистика по Issues:\n"
        report += "Всего - {total}\nОткрытых - {open}\nСтарых - {old}\nЗакрытых - {closed}\n".format(
            **self.get_stats(self.issues)
        )
